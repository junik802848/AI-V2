module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!item && page === "tbm75") { page = "tbm40"; const retry = await fetch(`${base}/storage/v1/object/list/tbm-files`, { method: "POST", headers: { Authorization: `Bearer ${key}`, apikey: key, "Content-Type": "application/json", "Cache-Control": "no-cache" }, body: JSON.stringify({ prefix: "tbm40/", limit: 1000 }) }); const retryRows = await retry.json(); const retryImages = Array.isArray(retryRows) ? retryRows.filter(x => /\.(png|jpe?g|webp|gif)$/i.test(String(x.name || ""))) : []; item = retryImages.sort((a,b) => Number(String(b.name||"").match(/^\d+/)?.[0]||0) - Number(String(a.name||"").match(/^\d+/)?.[0]||0))[0]; }
